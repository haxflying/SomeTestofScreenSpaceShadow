...
Shader "Hidden/Test-ScreenSpaceShadows" {
Properties {
...
v2f vert (appdata v)
{
    ...
    o.customRay = mul(unity_CameraInvProjection, float4((float2(v.texcoord.x, v.texcoord.y) - 0.5) * 2, -1, -1));
    // Perspective case
#ifdef UNITY_STEREO_INSTANCING_ENABLED
...
inline float3 computeCameraSpacePosFromDepthAndVSInfo(v2f i)
{
    ...
    #if USECUSTOMRAY
    float3 vposPersp = i.customRay * depth;
    #else
...
fixed4 frag_hard (v2f i) : SV_Target
{
    ...
    i.customRay *= (_ProjectionParams.z / i.customRay.z);
    ...
    //1 tap hard shadow
    #if USESHADOWMAPCOPY
    fixed shadow = tex2D(ShadowmapCopy, shadowCoord.xy) < shadowCoord.z;
    #else
...
SubShader {
    ...
    Pass {
        ...
        #pragma multi_compile __ USESHADOWMAPCOPY
        #pragma multi_compile __ USECUSTOMRAY
        #pragma multi_compile __ USEINVPROJ
